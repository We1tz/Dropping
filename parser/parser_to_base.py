...
def dataset_clear(vac):
    ...
    for row in vac["Vacancy"]:
        ...
        for j in range (len(row)):
            ...
            row[j] = row[j].strip()
            if str(morph.parse(row[j])[0].tag.POS) in ["PREP", "CONJ", "PRCL"] or row[j] in ru_stopwords or re.sub(r"\s+", "", row[j], flags=re.UNICODE).isdigit():
                row[j] = ""
            else:
...
def get_vacancy_text(url):
    ...
    if "hh.ru" in url:
        ...
        text = soup.find("div", "style-titleWrapper-Hmr_5").text + " " + soup.find("span","styles-module-size_xxxl-GRUMY").text
